                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr
                                ...
                                <th class="px-8 py-5">Nama Ruangan</th>
                                <th class="px-8 py-5">Total Aset</th>
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                            @foreach ($data as $room)
                                <tr class="group hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors">
                                    ...
                                    <td class="px-8 py-6">
                                        <span class="text-sm text-slate-600 dark:text-slate-400 px-3 py-1 rounded-lg">
                                            {{ $room->item_count }} Barang
                                        </span>
                                    </td>
                                    <td class="px-8 py-6">
                                        <span
                                            class="inline-flex items-center text-[10px] font-black uppercase tracking-widest {{ $room->size === 'small' ? 'text-emerald-500' : ($room->size === 'medium' ? 'text-amber-500' : 'text-rose-500') }}">
                                            <span
                                                class="w-2 h-2 rounded-full mr-2 animate-pulse {{ $room->size === 'small' ? 'bg-emerald-500' : ($room->size === 'medium' ? 'bg-amber-500' : 'bg-rose-500') }}"></span>
                                            {{ $room->size }}
                                        </span>
                                    </td>
                                    <td class="px-8 py-6 text-right">
                                        <a href="{{ route('room.show', $room->uuid) }}"
